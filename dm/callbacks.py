...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, epoch=-1, save_multiple=False, save_all=False, val_psnr=None, median_r=None, median_t=None):
        ...
        if self.val_on_psnr==False and median_t != None:
            ...
            if self.best_score is None: #一开始先进来，然后就不进了
                self.best_score = score
                self.save_checkpoint(median_t, model, epoch=epoch, save_multiple=save_multiple)
            elif score < self.best_score + self.delta:
                self.counter += 1
                ...
                if save_all: # save all ckpt #save_all = False in dfnetdm training
                    self.save_checkpoint(median_t, model, epoch=epoch, save_multiple=True, update_best=False)
            else: # save best ckpt only
                self.best_score = score
                self.save_checkpoint(median_t, model, epoch=epoch, save_multiple=save_multiple)
                self.counter = 0
            ...
            print('\n\n ============= self.val_on_psnr = ', self.val_on_psnr,'validation on val_loss!! (in /home/jialu/zeroshot123cycle0524/dm/callbacks.py)')
            score = -val_loss
            if self.best_score is None:
                self.best_score = score
