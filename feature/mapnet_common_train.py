...
import feature.mapnet_common_Logger as Logger
  ...
  def train_val(self, lstm):
    ...
    for epoch in range(self.start_epoch, self.config['n_epochs']):
        ...
        for batch_idx, (data, target) in enumerate(self.val_loader):
          ...
          if batch_idx % self.config['print_freq'] == 0:
            print('Val {:s}: Epoch {:d}\t' \
                  ...
                  'Data time {:.4f} ({:.4f})\t' \
                  'Batch time {:.4f} ({:.4f})\t' \
                  'Loss {:f}'.format(self.experiment, epoch, batch_idx, len(self.val_loader)-1, val_data_time.val, val_data_time.avg, val_batch_time.val, val_batch_time.avg, loss))
            if self.config['log_visdom']:
      ...
      if epoch % self.config['snapshot'] == 0:
        ...
        print('Epoch {:d} checkpoint saved for {:s}'.format(epoch, self.experiment))
      ...
      for batch_idx, (data, target) in enumerate(self.train_loader):
        ...
        if lstm:
          loss, _ = step_lstm(data, self.model, self.config['cuda'], **kwargs)
        else: #lstm =  False; data.shape= torch.Size([batch_size = 20, 6, 3, 256, 341]);target.shape= torch.Size([batch_size = 20, 5, 6])
          loss, _ = step_feedfwd(data, self.model, self.config['cuda'],
            **kwargs)
        ...
        if batch_idx % self.config['print_freq'] == 0:
          ...
          print('Train {:s}: Epoch {:d}\t' \
                ...
                'Batch Time {:.4f} ({:.4f})\t' \
                'Loss {:f}\t' \
                'lr: {:f}'.format(self.experiment, epoch, batch_idx, len(self.train_loader)-1, train_data_time.val, train_data_time.avg, train_batch_time.val, train_batch_time.avg, loss, lr))
          if self.config['log_visdom']:
...
def step_feedfwd(data, model, cuda, target=None, criterion=None, optim=None,
  ...
  :return: 
  """
  #zeroshot cycle =2的时候：train= False criterion =  None
  if train:
  ...
  with torch.set_grad_enabled(train): #eval.py时候train=False
    output = model(data_var)
  
  if criterion is not None:  #eval.py时候 criterion=None,不进来
    if cuda:
