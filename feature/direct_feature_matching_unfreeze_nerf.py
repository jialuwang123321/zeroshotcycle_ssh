...
from feature.get_atloc_model import get_atloc_vo_model
...
def train_on_batch(args, data, model, feat_model, pose, img_idx, hwf, optimizer, half_res, device, world_setup_dict,optimizer_nerf, **render_kwargs_train):
    ...
    if half_res:
        ...
        rgb, disp, acc, extras = render(H, W, focal, chunk=args.chunk, c2w=pose_nerf[0,:3,:4], img_idx=img_idx, **render_kwargs_train)
        rgb = rgb[None,...].permute(0,3,1,2)
    ...
    # loss_d = loss_func_nerf(results, target_s)
...
def train_on_epoch(args, data_loaders, model, feat_model, hwf, optimizer, half_res, device, world_setup_dict, optimizer_nerf, num_cycle=2, **render_kwargs_train ):
    ''' Perform 1 epoch of training with batch '''
    model.train()
    ...
    for data, pose, img_idx in train_dl:
        ...
        if shape[0] == 1: #1. 原版dfnet data.shape =  torch.Size([1, 3, 240, 320])
            ...
            #2.2 取VO模型, atloc_heads_vo
            mapnet_vo_model, vo_filename = get_atloc_vo_model(device)
            if print_information_once ==1:
                print('\n = = = =  Load vo weight from pretrained atloc_vo:', vo_filename, '\n')
                print_information_once =0
            ...
            # print('\n\n ===1 pa1.shape = ',pa1.shape , 'pa1.device = ', pa1.device)
            pa1pb1 = papb2vo(pa1, pb1, device) #pa1.dtype = torch.float32 # pa1pb1.dtype =  torch.float32
            ...
            pose_a_gt = pose_a.view(1, 3, 4).to(device)
            pose_b_gt = pose_b.view(1, 3, 4).to(device)
            # print('\n\n ===2 pose_a_gt.shape = ',pose_a_gt.shape , 'pose_a_gt.device = ', pose_a_gt.device)
            papb_gt = papb2vo(pose_a_gt, pose_b_gt, device)
            loss = cycle_1_loss_on_batch(optimizer, photo_loss_a01,photo_loss_b01, vo1, pa1pb1, papb_gt, device)
        else:  #3. 否则报错
...
def train_feature_matching(args, model, feat_model, optimizer, i_split, hwf, near, far, device, early_stopping, images=None, poses_train=None, train_dl=None, val_dl=None, test_dl=None, num_cycle=2):
    ...
    # load NeRF model
    # _, render_kwargs_test, start, grad_vars, optimizer_nerf = create_nerf(args)
    ...
    # # 获取posenet 模型的初始状态
    # initial_state_posenet_feat = feat_model.state_dict()

    # # 将初始状态保存为initial_state变量
    # torch.save(initial_state_posenet_feat, "initial_state_posenet_feat.pth")

    # # 获取posenet 模型的初始状态
    # initial_state_posenet = model.state_dict()

    # # 将初始状态保存为initial_state变量
    # torch.save(initial_state_posenet, "initial_state_posenet.pth")


    # # 获取NERF模型的初始状态
    # initial_state = render_kwargs_train['network_fn'].state_dict()

    # # 将初始状态保存为initial_state变量
    # torch.save(initial_state, "initial_state.pth")

    # # 获取NERF模型的初始状态
    # initial_state_network_fine = render_kwargs_train['network_fine'].state_dict()

    # # 将初始状态保存为initial_state变量
    # torch.save(initial_state_network_fine, "initial_state_network_fine.pth")
    ...
    for epoch in tqdm(range(N_epoch), desc='epochs'): #在这个循环里, half_res = True, N_epoch = 2001 in DFNET 
        ...
        ###### 在每个epoch结束后打印模型参数
        # print(f"\n\n\ ================ Epoch {epoch+1} Model Parameters:")
        # for name, param in model.named_parameters():
        #     if param.requires_grad:
        #         print(f"Posenet - {name}:")
        #         print(param.data)
        ...
        # # 在训练循环结束后进行验证
        # for name0, param0 in feat_model.named_parameters():
        #     if param0.requires_grad:
        #         if torch.allclose(param0, initial_state_posenet_feat[name0]):
        #             print(f"\n\n*************  feat_model's {name0} weights have not been updated.")
        #         else:
        #             print(f"\n\n*************  feat_model's {name0} weights have been updated.")

        for name2, param2 in feat_model.named_parameters():
        ...
        # for name1, param1 in model.named_parameters():
            ...
            #     print('param1 = ', param1, 'initial_state_posenet[name1] = ', initial_state_posenet[name1] )
            # if param1.requires_grad:
            #     if torch.allclose(param1, initial_state_posenet[name1]):
            #         print(f"\n\n*************  posenet model's {name1} weights have not been updated.")
            #         print('param1 = ', param1, 'initial_state_posenet[name1] = ', initial_state_posenet[name1] )
            #     else:
            #         print(f"\n\n*************  posenet model's {name1} weights have been updated.")
        ...
        # # 打印最后一层的名称和参数
        # print("posenet Last Layer Name:", last_layer_name)
        # print("posenet Last Layer Parameters:", last_layer_param)
   
        # for name2, param2 in render_kwargs_train['network_fn'].named_parameters():
        #     if param2.requires_grad:
        #         if torch.equal(param2, initial_state[name2]):
        #             print(f"\n\n*************  NERF model's {name2} weights have not been updated.")
        #         else:
        #             print(f"\n\n*************  NERF model's {name2} weights have been updated.")


        # for name3, param3 in render_kwargs_train['network_fine'].named_parameters():
        #     if param3.requires_grad:
        #         if torch.equal(param3, initial_state_network_fine[name3]):
        #             print(f"\n\n*************  network_fine model's {name3} weights have not been updated.")
        #         else:
        #             print(f"\n\n*************  network_fine model's {name3} weights have been updated.")

        # for name2, param2 in render_kwargs_train['network_fine'].named_parameters():
        #     if param2.requires_grad:
        #         last_layer_name = name2
        #         last_layer_param = param2

        # # 打印最后一层的名称和参数
        # print("NERF Last Layer Name:", last_layer_name)
        # print("NERF Last Layer Parameters:", last_layer_param)

        # # NOTE: IMPORTANT!
        # ###   update learning rate dfnetdm start ###

        # global_step += 1
        
        # decay_rate = 0.1
        # decay_steps = args.lrate_decay * 1000
        # new_lrate = args.learning_rate * (decay_rate ** (global_step / decay_steps))
        ...
        # NOTE: IMPORTANT!
        ###   update learning rate nerf start ###
        ...
        median_t, median_r = get_error_in_q(args, val_dl, model, len(val_dl.dataset), device, batch_size=1)
        early_stopping(val_loss, model, epoch=epoch, save_multiple=(not args.no_save_multiple), save_all=args.save_all_ckpt, val_psnr=val_psnr, median_r =median_r, median_t = median_t)
...
'''
def train_on_nerf(args, pa1, model, feat_model, pose, img_idx, hwf, optimizer, half_res, device, world_setup_dict, **render_kwargs_train):
    ''' 
...
def cycle_1_loss_on_batch(optimizer, photo_loss_a01, photo_loss_b01, vo1, vo_pa1b1,pa1pb_gt, device):
    
    photo_loss = photo_loss_a01 + photo_loss_b01

    loss_func = mapnet_vo_loss 
    vo_loss = loss_func(vo_pa1b1, pa1pb_gt)

    loss = 100*photo_loss - vo_loss
    # loss =  vo_loss
    # print('photo_loss_a01 = {}, photo_loss_b01 = {},\n loss =photo_loss+ vo_loss = {} + {}'.format(photo_loss_a01, photo_loss_b01, photo_loss, vo_loss))
    ### Loss Design End
    loss.backward()
    optimizer.step()
    optimizer.zero_grad()
...
def mapnet_vo_loss(pred_vos, targ_vos):
    ...
    # print('(pred_vos.view(-1, *s[2:])[:, :3]).shape = ', (pred_vos.view(-1, *s[2:])[:, :3]).shape)
    return vo_loss
