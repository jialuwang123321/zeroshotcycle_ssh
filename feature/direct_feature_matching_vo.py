...
def train_on_batch(args, data, model, feat_model, pose, img_idx, hwf, optimizer, half_res, device, world_setup_dict, **render_kwargs_test):
    ...
    # feature_list, _ = inference_pose_regression(args, torch.cat([data, rgb_2]), device, feat_model, retFeature=True, isSingleStream=False, return_pose=False)
    # feature_target = feature_list[0]
    # feature_rgb = feature_list[1]

    ### Loss Design Here ###
    # Compute RGB MSE Loss
    photo_loss = rgb_loss(rgb_2, data, extras_2)
    ...
    # # Compute 1st VO Loss
    #pose_loss_VO1 = PoseLoss_VO(args, rgb_2, pose, device)
    # # Compute 2nd VO Loss  
    ...
    optimizer.step()
    optimizer.zero_grad()
    psnr = mse2psnr(img2mse(rgb_2, data))
