    ...
    for batch_idx, (target, pose, img_idx) in enumerate(dl):
        ...
        with torch.no_grad():
            ...
            if args.tinyimg: #run_feature进来，args.tinyimg=True
                rgb, _, _, _ = render(int(H//args.tinyscale), int(W//args.tinyscale), focal/args.tinyscale, chunk=args.chunk, c2w=pose_nerf[0,:3,:4].to(device), retraw=True, img_idx=img_idx, **render_kwargs_test) #**render_kwargs_train
